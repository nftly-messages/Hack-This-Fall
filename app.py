...
def index():
    ...
    if payload and verifyToken(payload):
        return render_template('index.html', userdata=users.get_userdata(payload), posts=users.get_posts(num, page), num=num, page=page)
    else:
        return render_template('index.html', posts=users.get_posts(num, page), num=num, page=page)
